...
class RecommendationController extends Controller
{
        ...
        try {
            ...
            if ($response->serverError()) {
                return response()->json([
                    ...
                    'message'          => 'AI Service đang tạm thời không hoạt động (lỗi ' . $response->status() . ').',
                    'recommended_songs' => []
                ], 503);
            }
            ...
            // Timeout hoặc connection refused → AI đang offline
            \Illuminate\Support\Facades\Log::warning('AI Service offline (userRecommendations): ' . $e->getMessage());
            return response()->json([
                'success'           => false,
                'ai_service_online' => false,
            ...
            \Illuminate\Support\Facades\Log::error('AI RecommendationController error: ' . $e->getMessage());
            return response()->json([
                'success'           => false,
                'ai_service_online' => false,
                'message'           => 'Service Unavailable: AI Server Offline.',
    ...
    public function similarSongs($song_id)
    {
        ...
        try {
            ...
            // FastAPI trả về lỗi 5xx → AI đang offline
            if ($response->serverError()) {
                return response()->json([
                    'success'          => false,
                    'ai_service_online' => false,
            ...
            \Illuminate\Support\Facades\Log::warning('AI Service offline (similarSongs): ' . $e->getMessage());
            return response()->json([
                'success'           => false,
                'ai_service_online' => false,
                'message'           => 'Service Unavailable: AI Server Offline.',
            ...
            \Illuminate\Support\Facades\Log::error('AI RecommendationController similarSongs error: ' . $e->getMessage());
            return response()->json([
                'success'           => false,
                'ai_service_online' => false,
                'message'           => 'Service Unavailable: AI Server Offline.',
