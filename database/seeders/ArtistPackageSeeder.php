...
use App\Models\ArtistPackageFeature;
...
class ArtistPackageSeeder extends Seeder
{
    public function run(): void
    {
        ...
        foreach ($packages as $data) {
            ...
            $package = ArtistPackage::updateOrCreate(
                ['name' => $data['name']],
                $data
            );

            // Sync features into the normalized artist_package_features table
            $package->features()->delete();
            foreach ($features as $order => $text) {
                ArtistPackageFeature::create([
            ...
            $this->command->info("  ✓ {$package->name}: " . number_format($package->price) . ' đ/năm');
        }
