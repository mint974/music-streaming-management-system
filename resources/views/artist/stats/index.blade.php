...
@section('content')
    <div class="container-fluid px-0">
        <div class="d-flex justify-content-between align-items-center mb-4">
            <div>
                <h2 class="h3 fw-bold text-white mb-1"><i class="fa-solid fa-chart-line text-primary me-2"></i>Tổng quan hiệu
                    suất</h2>
                <p class="text-muted small">Cập nhật lúc: {{ now()->format('H:i d/m/Y') }}</p>
            </div>
            <button class="btn btn-sm btn-outline-secondary rounded-pill" onclick="window.location.reload()"><i
                    class="fa-solid fa-rotate-right me-1"></i>Làm mới</button>
        </div>
                    ...
                    <div class="d-flex justify-content-between align-items-start">
                        <div>
                            ...
                            <div class="trend-up"><i
                                    class="fa-solid fa-arrow-trend-up me-1"></i>+{{ number_format($todayListens) }} hôm nay
                            </div>
                        </div>
                ...
                <div class="stat-card">
                    <div class="d-flex justify-content-between align-items-start">
                        <div>
                            ...
                            <div class="text-muted small"><i class="fa-regular fa-calendar me-1"></i>Trong tháng
                                {{ now()->month }}</div>
                        </div>
                    ...
                    <div class="d-flex justify-content-between align-items-start">
                        <div>
                            ...
                            @if ($recentFollowers > 0)
                                <div class="trend-up"><i
                                        class="fa-solid fa-arrow-trend-up me-1"></i>+{{ number_format($recentFollowers) }}
                                    tuần này</div>
                            @else
        ...
        <div class="row g-4 mb-4">
            ...
            <div class="col-lg-8">
                <div class="chart-container h-100">
                    <h5 class="text-white fw-bold mb-4"><i class="fa-solid fa-chart-area me-2 text-muted"></i>Biểu đồ Lượt
                        nghe (30 ngày)</h5>
            ...
            <div class="col-lg-4">
                <div class="chart-container h-100">
                    <h5 class="text-white fw-bold mb-4"><i class="fa-solid fa-trophy me-2 text-warning"></i>Top 5 Bài hát
                        Phổ biến</h5>
                    @forelse ($topSongs as $index => $tsr)
                        <div class="top-song-item">
                            <div class="top-song-idx">#{{ $index + 1 }}</div>
                            <img src="{{ $tsr->cover_image ? asset($tsr->cover_image) : asset('storage/disk.png') }}"
...
@push('scripts')
    ...
    <script>
        ...
        let sLabels = Object.keys(sourceDist).map(l => l === 'stream' ? 'Nghe trực tuyến' : (l === 'download' ?
            'Tải offline' : l));
