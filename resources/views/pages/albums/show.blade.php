                        ...
                        <div class="card-body song-detail-hero-body">
                            ...
                            <div class="detail-chip-row mb-4">
                                ...
                                @if ($album->released_date)
                                    <span class="detail-chip">
                                        <i
                                            class="fa-regular fa-calendar me-1"></i>{{ $album->released_date->format('d/m/Y') }}
                                    </span>
                                @endif
                                <span class="detail-chip">
                            ...
                            <div class="detail-actions">
                                @if ($tracks->isNotEmpty())
                                    ...
                                    <button type="button" class="btn btn-song-play js-play-song"
                                        data-song-id="{{ $firstTrack->id }}" data-song-title="{{ e($firstTrack->title) }}"
                                ...
                                @endauth
                                @if ($canUseOffline)
                                    <button type="button" id="btnAlbumOffline" class="btn btn-outline-success px-3"
                                        onclick="syncAlbumOffline()">
                            ...
                            @if ($album->description)
                                <div class="info-row" style="flex-direction: column; align-items: flex-start; gap: .4rem;">
                                    ...
                                    <span class="info-value text-start"
                                        style="text-align:left!important; color: rgba(255,255,255,.78); font-size:.84rem; line-height:1.65;">
                                        {{ $album->description }}
                                    </span>
                                </div>
                            @endif
                    ...
                    <div class="card song-detail-card animate-on-scroll mb-4">
                        ...
                        <div class="card-body p-0">
                            @forelse($tracks as $index => $track)
                                <div class="tracklist-item stagger-item m-2  {{ $loop->first ? 'is-active' : '' }}">
                                    <div class="track-main">
                                        <img src="{{ $track->getCoverUrl() }}" alt="{{ $track->title }}">
                                        <div style="min-width:0;">
                                            <a class="name"
                                                href="{{ route('songs.show', $track->id) }}">{{ $track->title }}</a>
                                            <div class="artist">
                                    ...
                                    <div class="d-flex align-items-center gap-2">
                                        <button type="button" class="btn btn-sm btn-song-play js-play-song"
                                            data-song-id="{{ $track->id }}" data-song-title="{{ e($track->title) }}"
                            ...
                            @empty
                                <div class="p-4 text-center text-muted">
                                    <i class="fa-solid fa-compact-disc mb-2 d-block"
                    ...
                    @if ($artistOtherAlbums->count() > 0)
                        <div class="card song-detail-card animate-on-scroll mb-4">
                            <div class="card-header d-flex align-items-center justify-content-between">
                                ...
                                <a href="{{ route('albums.index', ['q' => $artistName]) }}"
                                    class="btn btn-sm btn-song-detail">Xem tất cả</a>
                            </div>
                            <div class="card-body">
                            ...
                            <div class="artist-side-head">
                                ...
                                <div class="flex-grow-1">
                                    ...
                                    <div class="artist-side-sub">
                                        {{ number_format((int) ($album->artist?->followers()->count() ?? 0)) }} người theo
                                        dõi
                                    </div>
                                </div>
                            </div>
                            @if ($album->artist?->bio)
                                <div class="artist-side-bio mt-3">
                                    {{ \Illuminate\Support\Str::limit($album->artist->bio, 140) }}</div>
                            @endif
                        </div>
                    </div>
                    ...
                    <div class="card song-detail-card animate-on-scroll tracklist-card">
                        ...
                        <div class="card-body p-0">
                            @foreach ($tracks->take(8) as $item)
                                <div class="tracklist-item stagger-item {{ $loop->first ? 'is-active' : '' }}">
                                    <div class="track-main">
                                        <img src="{{ $item->getCoverUrl() }}" alt="{{ $item->title }}">
                                        <div>
                                            <a class="name"
                                                href="{{ route('songs.show', $item->id) }}">{{ $item->title }}</a>
                                            <div class="artist">
                                    ...
                                    <div class="d-flex align-items-center gap-2">
                                        <button type="button" class="btn btn-sm btn-song-play js-play-song"
                                            data-song-id="{{ $item->id }}" data-song-title="{{ e($item->title) }}"
...
@push('scripts')
    <script>
        ...
        async function syncAlbumOffline() {
            ...
            try {
                await window.BWMOffline.syncUrls(albumOfflineUrls, ({
                    done,
                    total
                }) => {
                    btn.innerHTML =
                        `<i class="fa-solid fa-spinner fa-spin me-1"></i>Đang tải (${done}/${total})`;
                });
