...
.p-curr {
    font-size: 1rem; color: #a1a1aa; font-weight: 500;
}
.p-desc {
    color: #a1a1aa;
    font-size: 0.9rem;
...
.btn-g.active-st {
    background: rgba(255,255,255,0.05); color: #8b5cf6; border: 1px solid rgba(139, 92, 246, 0.4);
    pointer-events: none;
}
...
<div class="sub-page py-5">
    <div class="container" style="max-width: 1100px;">
        ...
        {{-- Flash messages --}}
        @if(session('success'))
        <div class="alert text-white mb-4" style="background: rgba(52,211,153,0.15); border: 1px solid rgba(52,211,153,0.3); border-radius: 12px;">
            <i class="fa-solid fa-circle-check me-2 text-success"></i>{!! session('success') !!}
        </div>
        @endif
        @if(session('error'))
        <div class="alert text-white mb-4" style="background: rgba(248,113,113,0.15); border: 1px solid rgba(248,113,113,0.3); border-radius: 12px;">
        ...
        <div class="text-center fade-in-up">
            ...
            <p class="hero-subtitle">Nghe nhạc không quảng cáo, tải xuống offline và chất lượng âm thanh tốt nhất. Khai phá sức mạnh giải trí vô hạn.</p>
        </div>
        ...
        <div class="status-banner fade-in-up delay-1 mx-auto max-w-3xl mt-5">
            <div class="d-flex align-items-center justify-content-between flex-wrap gap-4">
                <div class="d-flex align-items-center gap-3">
                    <div style="width: 50px; height: 50px; background: var(--gemini-gradient); border-radius: 12px; display:flex; align-items:center; justify-content:center;">
                        <i class="fa-solid fa-gem text-white fs-4"></i>
                    </div>
                    <div>
                        <h5 class="text-white fw-bold mb-1">Gói hiện tại: {{ $activeSub->vip->title }}</h5>
                        <div class="text-muted small">Thời hạn từ {{ $activeSub->start_date->format('d/m/Y') }} đến {{ $activeSub->end_date->format('d/m/Y') }}</div>
                    </div>
                </div>
                <div class="text-end">
        ...
        <div class="pricing-grid fade-in-up delay-2">
            ...
            <div class="gemini-card {{ $isRecommended ? 'glow-card' : 'standard-card' }}">
                @if($isRecommended)
                    <div class="position-absolute top-0 end-0 mt-3 me-3 text-white px-3 py-1 fw-bold rounded-pill" style="font-size: 0.75rem; background: var(--gemini-gradient);">ĐỀ XUẤT</div>
                @endif
                
                <h3 class="p-name">
                    @if($isRecommended) <i class="fa-solid fa-bolt" style="color: #f472b6;"></i> @else <i class="fa-regular fa-circle-play" style="color: #8b5cf6;"></i> @endif
                    {{ $vip->title }}
                </h3>
                <p class="p-desc">{{ $vip->description ?: 'Trải nghiệm toàn bộ ưu đãi của nền tảng với mức giá tiết kiệm.' }}</p>
                ...
                <ul class="p-features">
                    ...
                    <li><i class="fa-solid fa-check"></i> Xem toàn bộ lời bài hát chi tiết</li>
        ...
        <div class="mt-5">
            <h5 class="text-white mb-3"><i class="fa-solid fa-clock-rotate-left me-2 text-muted"></i>Lịch sử thanh toán</h5>
                 ...
                 <table class="table table-dark table-hover mb-0 align-middle">
                     ...
                     <tbody>
                         ...
                         <tr>
                             ...
                             <td>
                                 <div class="small">{{ $sub->start_date->format('d/m/Y') }} → {{ $sub->end_date->format('d/m/Y') }}</div>
                             </td>
                             <td class="text-end fw-semibold text-warning">
                                 {{ number_format($sub->amount_paid) }} ₫
                             </td>
                             <td class="text-center">
                                 <span class="badge {{ $sub->status === 'active' ? 'bg-success' : ($sub->status === 'pending' ? 'bg-warning text-dark' : 'bg-secondary') }}">
                                     {{ $sub->statusLabel() }}
                                 </span>
                             </td>
                             <td class="text-center">
                                 @if($sub->payment)
                                 <span class="badge border {{ $sub->payment->status === 'paid' ? 'border-success text-success' : 'border-secondary text-muted' }} bg-transparent">
