...
@section('page-subtitle', 'Cập nhật thông tin hiển thị banner trang chủ')
        ...
        <form action="{{ route('admin.banners.update', $banner->id) }}" method="POST" enctype="multipart/form-data">
            ...
            <div class="card bg-dark border-secondary border-opacity-25 shadow-sm">
                <div class="card-header bg-transparent border-secondary border-opacity-25 py-3 d-flex justify-content-between align-items-center">
                    <span class="text-white fw-semibold"><i class="fa-solid fa-pen-to-square me-2 text-warning"></i>Sửa Banner #{{ $banner->id }}</span>
                    ...
                    <div class="row g-4">
                        <div class="col-12 col-lg-7">
                            ...
                            <div class="mb-3">
                                ...
                                <input type="text" name="title" class="form-control bg-dark border-secondary text-white shadow-none @error('title') is-invalid @enderror" value="{{ old('title', $banner->title) }}" required>
                            ...
                            <div class="row g-3 mb-3">
                                <div class="col-md-6">
                                    ...
                                    <input type="datetime-local" name="start_time" class="form-control bg-dark border-secondary text-white shadow-none" value="{{ old('start_time', $banner->start_time ? $banner->start_time->format('Y-m-d\\TH:i') : '') }}">
                                </div>
                                <div class="col-md-6">
                                    <label class="form-label text-muted small">Kết thúc hiển thị <span class="text-muted">(Optional)</span></label>
                                    <input type="datetime-local" name="end_time" class="form-control bg-dark border-secondary text-white shadow-none" value="{{ old('end_time', $banner->end_time ? $banner->end_time->format('Y-m-d\\TH:i') : '') }}">
                                </div>
                            </div>
                            ...
                            <div class="row g-3">
                                <div class="col-md-6">
                                    ...
                                    <select name="status" class="form-select bg-dark border-secondary text-white shadow-none @error('status') is-invalid @enderror" required>
                                        <option value="active" {{ old('status', $banner->status) === 'active' ? 'selected' : '' }}>Đang bật</option>
                                        <option value="inactive" {{ old('status', $banner->status) === 'inactive' ? 'selected' : '' }}>Tạm tắt</option>
                                    </select>
                                </div>
                                <div class="col-md-6">
                                    <label class="form-label text-muted small">Thứ tự hiển thị <span class="text-danger">*</span></label>
                                    <input type="number" name="order_index" class="form-control bg-dark border-secondary text-white shadow-none" value="{{ old('order_index', $banner->order_index) }}" min="0" required>
                                </div>
                            </div>
                        </div>
                        ...
                        <div class="col-12 col-lg-5">
                            ...
                            <div class="p-4 rounded border border-secondary border-opacity-50 text-center" style="background: rgba(255,255,255,.02); position: relative; overflow: hidden;">
                                <div id="imagePreviewContainer" class="mb-3 position-relative">
                                    <img id="imagePreview" src="{{ asset($banner->image_path) }}" alt="Preview" class="img-fluid rounded w-100 shadow-sm" style="max-height: 220px; object-fit: contain; background: #000;">
                                ...
                                <label class="form-label text-white d-block mb-1">Thay đổi ảnh banner</label>
                                <div class="form-text text-muted mb-3" style="font-size:.75rem">Để trống nếu muốn giữ ảnh hiện tại.</div>
